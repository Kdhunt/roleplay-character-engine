...
Ownership is a stored edge rather than an inference: continuity-scoped rows carry owner, continuity and branch identifiers, and foreign keys crossing into continuity data are composite keys including the owner, so a mismatched parent is a constraint violation and not merely a policy check a query could omit. The atomic commit boundary resolves the gap ADR-004 left open by testing membership rather than record type: a derived fact commits with the turn only when a pure reducer over the accepted events produces it and the next turn cannot be validated without it. Anything whose computation can fail for a reason unrelated to the transaction is deferred, made idempotent and tracked by a checkpoint. Every event is stored as separately-erasable content alongside retained integrity metadata, so an append-only log still permits erasure; reducers must tolerate a tombstoned payload and mark the resulting state incomplete rather than fail or invent a value. Retention durations, log partitioning and snapshot cadence remain recorded owner decisions.
